...
describe(`Multicall`, async () => {
  ...
  beforeEach(`create multicall`, async () => {
    const multicallTestFactory = await ethers.getContractFactory(
      `TestMulticall`
    );
  ...
  it(`revert messages are returned`, async () => {
    await expect(
      multicall.multicall([
        multicall.interface.encodeFunctionData(`functionThatRevertsWithError`, [
          `abcdef`,
        ]),
      ])
  ...
  it(`return data is properly encoded`, async () => {
    const [data] = await multicall.callStatic.multicall([
      multicall.interface.encodeFunctionData(`functionThatReturnsTuple`, [
        `1`,
        `2`,
      ]),
    ]);
  ...
  it(`gas cost of pay w/ multicall`, async () => {
    await snapshotGasCost(
      multicall.multicall([multicall.interface.encodeFunctionData(`pays`)], {
        value: 3,
      })
    );
  });
