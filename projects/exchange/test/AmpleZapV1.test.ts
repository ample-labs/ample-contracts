...
contract(`AmpleZapV1`, ([alice, bob, carol, david, erin]) => {
  ...
  before(async () => {
    ...
    // Deploy Router
    ampleRouter = await AmpleRouter.new(
    ...
    // Create 3 LP tokens
    let result = await ampleFactory.createPair(
      tokenA.address,
      wrappedETH.address,
      { from: alice }
    );
    ...
    result = await ampleFactory.createPair(wrappedETH.address, tokenC.address, {
      from: alice,
    });
    pairBC = await AmplePair.at(result.logs[0].args[2]);

    result = await ampleFactory.createPair(tokenA.address, tokenC.address, {
      from: alice,
    });
  ...
  describe(`Normal cases for liquidity provision and zap ins`, async () => {
    it(`User adds liquidity to LP tokens`, async function () {
      ...
      assert.equal(
        String(await wrappedETH.balanceOf(pairAB.address)),
      ...
      assert.equal(
        String(await wrappedETH.balanceOf(pairBC.address)),
        parseEther(`1000`).toString()
      );
      assert.equal(
    ...
    it(`User completes zapIn with tokenA (pair tokenA/tokenC)`, async function () {
      ...
      const estimation = await ampleZap.estimateZapInSwap(
        tokenToZap,
        parseEther(`1`),
        lpToken
      );
      ...
      const result = await ampleZap.zapInToken(
        tokenToZap,
        tokenAmountIn,
        lpToken,
        minTokenAmountOut,
      ...
      console.info(
        `Balance tokenA: ${formatUnits(
          String(await tokenA.balanceOf(ampleZap.address)),
          18
        )}`
      );
      console.info(
        `Balance WETH: ${formatUnits(
          String(await wrappedETH.balanceOf(ampleZap.address)),
          18
        )}`
      );
      console.info(
        `Balance tokenC: ${formatUnits(
          String(await tokenC.balanceOf(ampleZap.address)),
          18
        )}`
      );
    });
    ...
    it(`User completes zapOut to token (tokenA/tokenC)`, async function () {
      ...
      const estimation = await ampleZap.estimateZapOutSwap(
        lpToken,
        lpTokenAmount,
        tokenToReceive
      );
      ...
      const result = await ampleZap.zapOutToken(
        lpToken,
        tokenToReceive,
        lpTokenAmount,
        minTokenAmountOut,
    ...
    it(`User completes zapOut to ETH (ETH/tokenC)`, async function () {
      ...
      const result = await ampleZap.zapOutETH(
        lpToken,
        lpTokenAmount,
        minTokenAmountOut,
        {
    ...
    it(`Zap estimations work as expected`, async function () {
      ...
      const estimation2 = await ampleZap.estimateZapInSwap(
        tokenA.address,
        parseEther(`5`),
        pairAC.address
      );
    ...
    it(`Cannot zap if wrong direction/tokens used`, async function () {
      ...
      await expectRevert(
        ampleZap.zapInETH(pairAC.address, parseEther(`0.51`), {
          from: carol,
      ...
      // David gets WETH
      const result = await wrappedETH.deposit({
        from: david,
