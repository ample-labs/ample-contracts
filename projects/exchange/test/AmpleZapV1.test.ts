...
const AmpleFactory = artifacts.require("./AmpleFactory.sol");
const AmplePair = artifacts.require("./AmplePair.sol");
const AmpleRouter = artifacts.require("./AmpleRouter.sol");
const AmpleZapV1 = artifacts.require("./AmpleZapV1.sol");
const WETH = artifacts.require("./WETH.sol");

contract("AmpleZapV1", ([alice, bob, carol, david, erin]) => {
  let maxZapReverseRatio;
  let pairAB: Contract;
  let pairBC: Contract;
  let pairAC: Contract;
  let ampleZap: Contract;
  let ampleRouter: Contract;
  let ampleFactory: Contract;
  let tokenA: Contract;
  let tokenC: Contract;
  ...
  before(async () => {
    // Deploy Factory
    ampleFactory = await AmpleFactory.new(alice, { from: alice });

    // Deploy Wrapped ETH
    wrappedETH = await WETH.new({ from: alice });

    // Deploy Router
    ampleRouter = await AmpleRouter.new(ampleFactory.address, wrappedETH.address, { from: alice });

    // Deploy ZapV1
    maxZapReverseRatio = 100; // 1%
    ampleZap = await AmpleZapV1.new(wrappedETH.address, ampleRouter.address, maxZapReverseRatio, { from: alice });
    ...
    // Create 3 LP tokens
    let result = await ampleFactory.createPair(tokenA.address, wrappedETH.address, { from: alice });
    pairAB = await AmplePair.at(result.logs[0].args[2]);

    result = await ampleFactory.createPair(wrappedETH.address, tokenC.address, { from: alice });
    pairBC = await AmplePair.at(result.logs[0].args[2]);

    result = await ampleFactory.createPair(tokenA.address, tokenC.address, { from: alice });
    pairAC = await AmplePair.at(result.logs[0].args[2]);
    ...
    for (let thisUser of [alice, bob, carol, david, erin]) {
      ...
      await tokenA.approve(ampleRouter.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await tokenA.approve(ampleZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await tokenC.approve(ampleRouter.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await tokenC.approve(ampleZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await wrappedETH.approve(ampleRouter.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await wrappedETH.approve(ampleZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await pairAB.approve(ampleZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await pairBC.approve(ampleZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await pairAC.approve(ampleZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });
    }
  });
  ...
  describe("Normal cases for liquidity provision and zap ins", async () => {
    it("User adds liquidity to LP tokens", async function () {
      ...
      // 1 A = 1 C
      let result = await ampleRouter.addLiquidity(
        tokenC.address,
        tokenA.address,
      ...
      // 1 ETH = 100 A
      result = await ampleRouter.addLiquidityETH(
        tokenA.address,
    ...
    it("User completes zapIn with tokenA (pair tokenA/tokenC)", async function () {
      ...
      const estimation = await ampleZap.estimateZapInSwap(tokenToZap, parseEther("1"), lpToken);
      ...
      const result = await ampleZap.zapInToken(tokenToZap, tokenAmountIn, lpToken, minTokenAmountOut, {
        from: carol,
      });
      ...
      console.info("Balance tokenA: " + formatUnits(String(await tokenA.balanceOf(ampleZap.address)), 18));
      console.info("Balance WETH: " + formatUnits(String(await wrappedETH.balanceOf(ampleZap.address)), 18));
      console.info("Balance tokenC: " + formatUnits(String(await tokenC.balanceOf(ampleZap.address)), 18));
    });
    ...
    it("User completes zapIn with ETH (pair ETH/tokenC)", async function () {
      ...
      const estimation = await ampleZap.estimateZapInSwap(wrappedETH.address, parseEther("1"), lpToken);
      assert.equal(estimation[2], tokenC.address);

      // Setting up slippage at 0.5%
      const minTokenAmountOut = new BN(estimation[1].toString()).mul(new BN("9995")).div(new BN("10000"));

      const result = await ampleZap.zapInETH(lpToken, minTokenAmountOut, {
        from: carol,
    ...
    it("User completes zapInRebalancing with ETH (pair ETH/tokenC)", async function () {
      ...
      const estimation = await ampleZap.estimateZapInRebalancingSwap(
        wrappedETH.address,
        tokenC.address,
        token0AmountIn,
        token1AmountIn,
      ...
      const result = await ampleZap.zapInETHRebalancing(
        tokenC.address,
        token1AmountIn,
        lpToken,
        maxTokenAmountIn,
    ...
    it("User completes zapInRebalancing with tokens (tokenA/tokenC)", async function () {
      ...
      const result = await ampleZap.zapInTokenRebalancing(
        tokenA.address,
        tokenC.address,
        token0AmountIn,
        token1AmountIn,
    ...
    it("User completes zapOut to token (tokenA/tokenC)", async function () {
      ...
      const estimation = await ampleZap.estimateZapOutSwap(lpToken, lpTokenAmount, tokenToReceive);
      assert.equal(estimation[2], tokenC.address);

      const minTokenAmountOut = new BN(estimation[1].toString()).mul(new BN("9995")).div(new BN("10000"));

      const result = await ampleZap.zapOutToken(lpToken, tokenToReceive, lpTokenAmount, minTokenAmountOut, {
        from: carol,
      });
    ...
    it("User completes zapOut to ETH (ETH/tokenC)", async function () {
      ...
      const result = await ampleZap.zapOutETH(lpToken, lpTokenAmount, minTokenAmountOut, {
        from: carol,
      });
    ...
    it("Zap estimation fail if wrong tokens", async function () {
      await expectRevert(
        ampleZap.estimateZapInSwap(wrappedETH.address, parseEther("1"), pairAC.address),
        "Zap: Wrong tokens"
      );
      await expectRevert(
        ampleZap.estimateZapInRebalancingSwap(
          tokenA.address,
          wrappedETH.address,
          parseEther("1"),
          parseEther("1"),
      ...
      await expectRevert(
        ampleZap.estimateZapOutSwap(pairAC.address, parseEther("1"), wrappedETH.address),
        "Zap: Token not in LP"
      );
    });
    ...
    it("Zap estimations work as expected", async function () {
      ...
      const estimation0 = await ampleZap.estimateZapInRebalancingSwap(
        tokenA.address,
        tokenC.address,
        parseEther("0.5"),
        parseEther("1"),
      ...
      const estimation1 = await ampleZap.estimateZapInRebalancingSwap(
        tokenC.address,
        tokenA.address,
        parseEther("1"),
        parseEther("0.5"),
      ...
      const estimation2 = await ampleZap.estimateZapInSwap(tokenA.address, parseEther("5"), pairAC.address);
      const estimation3 = await ampleZap.estimateZapInRebalancingSwap(
        tokenA.address,
        tokenC.address,
        parseEther("5"),
        parseEther("0"),
    ...
    it("Cannot zap if wrong direction/tokens used", async function () {
      await expectRevert(
        ampleZap.zapInToken(tokenA.address, parseEther("1"), pairBC.address, parseEther("0.51"), { from: carol }),
        "Zap: Wrong tokens"
      );
      await expectRevert(
        ampleZap.zapInETH(pairAC.address, parseEther("0.51"), { from: carol, value: parseEther("0.51").toString() }),
        "Zap: Wrong tokens"
      );

      await expectRevert(
        ampleZap.zapOutToken(pairBC.address, tokenA.address, parseEther("0.51"), parseEther("0.51"), { from: carol }),
        "Zap: Token not in LP"
      );

      await expectRevert(
        ampleZap.zapOutETH(pairAC.address, parseEther("0.51"), parseEther("0.51"), { from: carol }),
        "Zap: Token not in LP"
      );

      await expectRevert(
        ampleZap.zapInTokenRebalancing(
          tokenA.address,
          tokenC.address,
          parseEther("1"),
          parseEther("1"),
